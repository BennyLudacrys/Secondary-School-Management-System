...
public class ClassRoomService {
    ...
    public String removeClass(Long idClass) {     //  Método apaga uma classe do sistema.
        ...
        try {
            ...
            throw new DatabaseException("You cannot delete a class that has both students and teachers. delete them first!");
        }
    ...
    private Student returnStudent(ClassRoom classRoom, Long idStudent, boolean addMethod) {   // boolean addMethod - Se for o metodo de adicionar chamando,
        ...
        if (!addMethod) {  // SE NÃO FOR O MÉTODO DE ADICIONAR
            studentRepository.findStudentByClassId(classRoom.getId(), idStudent)// Pesquisa se o aluno existe nessa CLASSE!
                    .orElseThrow(() -> new StudentDoesntExistInThisClassException("This student is not in this class")); // Se não existir, ERRO!
        }
        return studentOptionalDataBase.orElseThrow(() -> new ResourceNotFoundException("Id : " + idStudent + ", This student was not found in the database"));
    }
    ...
    private Teacher returnTeacher(Long idTeacher) { // Método que retorna um Professor do banco.
        ...
        return teacher.orElseThrow(() -> new ResourceNotFoundException("Id : " + idTeacher + ", This teacher is not in the database"));
    }
